document.addEventListener('DOMContentLoaded', function() {
    ...
    const menuData = {
        "MENU KOMPLIT": {
            ...
            items: [ { name: "Nasi Ayam Kremes (Paha Potong)", price: 20000 }, { name: "Nasi Ayam Kremes Kampung (Paha/Dada)", price: 30000 }, { name: "Nasi Tempong Ayam Kampung (Paha/Dada)", price: 25000 }, { name: "Nasi Tempong Polos", price: 12000 }, { name: "Nasi Garang Asem (Paha/Dada)", price: 30000 }, { name: "Nasi Ikan Pe Goreng", price: 16000 }, { name: "Nasi Ikan Pe Kuah", price: 18000 }, { name: "Nasi Ayam Bakar Kampung (Paha/Dada)", price: 35000 }, ]
        },
        ...
        "CHINESE FOOD & SEAFOOD": { note: null, items: [ { name: "Nasi Hainan Ayam Panggang", price: 32000 }, { name: "Nasi Hainan Ayam Rebus", price: 30000 }, { name: "Ayam Panggang 1 Ekor", price: 110000 }, { name: "Ayam Panggang 1/2 Ekor", price: 65000 }, { name: "Ayam Rebus 1 Ekor", price: 95000 }, { name: "Ayam Rebus 1/2 Ekor", price: 45000 }, { name: "Gurami Jimbaran Besar", price: 75000 }, { name: "Gurami Jimbaran Sedang", price: 60000 }, { name: "Gurami Goreng Dabu - Dabu Besar", price: 75000 }, { name: "Gurami Goreng Dabu - Dabu Sedang", price: 60000 }, { name: "Udang Bakar Jimbaran", price: 40000 }, { name: "Kerang Bakar Jimbaran", price: 40000 }, ] },
    ...
    function renderMenu() {
        for (const category in menuData) {
            ...
            accordionItem.className = 'accordion-item bg-white overflow-hidden';

            const itemsHTML = categoryData.items.map(item => {
                const priceFormatted = (item.price / 1000) + 'K';
                return `
                    <div class="menu-item flex items-center justify-between py-4 border-b border-gray-100 last:border-b-0">
                        <div class="mr-4">
                            <p class="font-semibold text-brand-brown">${item.name}</p>
                            <p class="text-sm text-brand-orange font-semibold">${formatRupiah(item.price)}</p>
                        </div>
                        <div class="quantity-selector flex items-center border border-gray-200 rounded-lg">
                            <button class="quantity-minus text-brand-red">-</button>
                            <input type="number" class="quantity-input focus:outline-none" value="0" min="0" data-name="${item.name}" data-price="${item.price}">
                            <button class="quantity-plus text-brand-red">+</button>
                        </div>
                    </div>
                `;
            }).join('');
            ...
            accordionItem.innerHTML = `
                <button class="accordion-toggle w-full flex justify-between items-center p-6 text-left">
                    <div>
                        <h3 class="font-poppins font-bold text-xl text-brand-brown">${category}</h3>
    ...
    function renderCart(items) {
        if (items.length === 0) {
            ...
            cartItemsContainer.innerHTML = items.map(item => `
                <div class="cart-item flex justify-between items-center animate-fade-in">
                    <div class="flex-1 pr-2">
                        <p class="font-semibold text-brand-brown cart-item-name">${item.name}</p>
                        <p class="text-sm text-gray-500">${item.quantity} &times; ${formatRupiah(item.price)}</p>
                    </div>
                    <p class="font-semibold text-brand-brown">${formatRupiah(item.quantity * item.price)}</p>
                </div>
            `).join('');
        }
    }
    ...
    renderMenu();
    updateOrder(); // Panggil sekali di awal untuk inisialisasi keranjang
});
